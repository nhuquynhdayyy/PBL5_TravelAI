...
import { Bot, CalendarCheck, Map, MessageCircle, Send, X } from 'lucide-react';
...
const initialMessage: ChatboxMessage = {
  text: 'Xin chào! Tôi là trợ lý TravelAI. Bạn muốn đi đâu?',
  sender: 'ai',
  type: 'text',
...
const Chatbox = () => {
  ...
  const buildHistory = (items: ChatboxMessage[]): ChatHistoryMessage[] =>
    items
      .filter(msg => msg.includeInHistory !== false && msg.text.trim().length > 0)
      .map(msg => ({
  ...
  const handleSend = async () => {
    ...
    const userMsg: ChatboxMessage = { text: trimmedInput, sender: 'user', type: 'text' };
