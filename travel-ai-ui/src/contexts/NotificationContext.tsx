...
const normalizeApiNotification = (value: any): NotificationItem => ({
  ...
  title: String(value.title ?? getFallbackTitle(String(value.type ?? 'notification'))),
...
const getFallbackTitle = (type: string) => {
  if (type === notificationEventNames.paymentCounter) return 'Xác nhận thanh toán tại quầy';
  if (type === notificationEventNames.partnerBookingConfirmed) return 'Don hang moi';
  if (type === notificationEventNames.userBookingConfirmed) return 'Thanh toan da xac nhan';
  return 'Thong bao';
};
...
const normalizeRealtimeNotification = (type: string, payload: any): NotificationItem => ({
  ...
  title: String(payload?.title ?? getFallbackTitle(type)),
  type,
...
export const NotificationProvider = ({ children }: { children: ReactNode }) => {
  ...
  const pushNotification = useCallback((type: string, payload: any) => {
    ...
    if (type === notificationEventNames.paymentCounter && role === 'partner') return;
  ...
  useEffect(() => {
    ...
    const registerHandlers = (activeConnection: HubConnection) => {
      ...
      activeConnection.on(notificationEventNames.paymentCounter, (payload) => {
        pushNotification(notificationEventNames.paymentCounter, payload);
      });
    ...
    fetchNotifications();
    const refreshTimer = window.setInterval(fetchNotifications, 15000);
