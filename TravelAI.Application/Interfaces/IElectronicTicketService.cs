...
public interface IElectronicTicketService
{
    ...
    Task<PublicTicketDto?> GetPublicByCodeAsync(string ticketCode, CancellationToken cancellationToken = default);
    Task<VerifyTicketResponse> VerifyAsync(string qrPayload, int verifierUserId, bool markAsUsed, CancellationToken cancellationToken = default);
}
